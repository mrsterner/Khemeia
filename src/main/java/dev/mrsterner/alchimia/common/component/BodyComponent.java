...
public class BodyComponent implements IBody, AutoSyncedComponent, PlayerComponent<BodyComponent> {
    ...
    public void setBodyPart(BodyParts bodyPart, boolean b) {
        ...
        AlchimiaComponents.BODY_COMPONENT.sync(player);
    }
    ...
    public float getHeight(){
        if(!AlchimiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.RIGHTLEG) && !AlchimiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.LEFTLEG)){
            if(!AlchimiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.TORSO) && !AlchimiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.LEFTARM) && !AlchimiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.RIGHTARM)){
                return 0.55F;
            }
            return 1.2F;
        }
