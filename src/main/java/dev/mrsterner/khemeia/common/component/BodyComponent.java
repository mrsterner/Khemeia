...
public class BodyComponent implements IBody, AutoSyncedComponent, PlayerComponent<BodyComponent> {
    ...
    public void setBodyPart(BodyParts bodyPart, boolean b) {
        ...
        KhemeiaComponents.BODY_COMPONENT.sync(player);
    }
    ...
    public float getHeight(){
        if(!KhemeiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.RIGHTLEG) && !KhemeiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.LEFTLEG)){
            if(!KhemeiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.TORSO) && !KhemeiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.LEFTARM) && !KhemeiaComponents.BODY_COMPONENT.get(player).hasBodyPart(BodyParts.RIGHTARM)){
                return 0.55F;
            }
            return 1.2F;
        }
